...
function upgradeRequest(options, cb) {
  ...
  const { hostname } = url.parse(options.baseUrl)
  ...
  options.servername = hostnameIsIp ? '' : undefined
...
export default class Request {
   ...
   * @param {function} checkExpired - Check if this configuration is no longer valid
   */
  constructor(options, checkExpired?) {
